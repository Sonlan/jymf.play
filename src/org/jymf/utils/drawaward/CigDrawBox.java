 ...
 */
@Service("cigdrawbox")
public class CigDrawBox {

    private static Logger logger = LoggerFactory.getLogger(CigDrawBox.class);
    ...
    public static final String recordFilePath = System.getProperty("catalina.base")+"/logs/jymfplay/record/cigawardbox.properties";
    ...
    static {
        try {
            ...
            //InputStream resourceAsStream = CigDrawBox.class.getResourceAsStream("/cigawardbox.properties");
    ...
    private boolean randomDraw(){
        ...
        int target = random.nextInt(CigConfigBox.ratio);
        if (target == 0){
            return true;
        }
        return false;
    ...
    public boolean drawMain(){
        synchronized (CigDrawBox.class){
            count++;
            if (count > CigConfigBox.lablenum){
                update();
                return false;
            }

            if (count% CigConfigBox.gap == 1){
                isAwardAvailable = true;
            }

            if (isAwardAvailable){
                if (count% CigConfigBox.gap == CigConfigBox.luckynum){
